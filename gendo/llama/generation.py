...
from gendo.llama.model import ModelArgs, Transformer
from gendo.llama.tokenizer import Tokenizer
from gendo.llama.convert import convert_to_nested, convert_transformer
...
class Llama:
    ...
    def build(
        ...
        Args:
            model_dir (str): Path to the directory containing PyTorch checkpoint files.
        ...
        # load and adjust model arguments
        ...
        return Llama(model, variables, tokenizer)

    def __init__(self, model: Transformer, variables: dict, tokenizer: Tokenizer):
        self.model = model
    ...
    def generate(
        ...
        Returns:
            Tuple[List[List[int]], Optional[List[List[float]]]]: A tuple containin   # TODO: this is wrongg generated token sequences and, if logprobs is True, corresponding token log probabilities.
        ...
        for k, t in enumerate(prompt_tokens):
            tokens = tokens.at[k, : len(t)].set(jnp.asarray(t))
        if logprobs:
        ...
        for cur_pos in range(min_prompt_len, total_len):
            ...
            if temperature > 0:
                ...
                probs = nn.softmax(logits[:, -1] / temperature, axis=-1)
                rng, sample_rng = jax.random.split(rng, 2)
                next_token = sample_top_p(sample_rng, probs, top_p)
            else:
...
def main():
    ...
    llama = Llama.build(model_dir, tokenizer_path, max_seq_len=512, max_batch_size=1)
    self = llama
    ...
    # tokens = self.tokenizer.encode("frankenstein walks into a bar", False, False)
    # tokens = jnp.asarray(tokens).reshape(1, -1)

    out_tokens = Llama.generate(self, prompt_tokens, 20)
    self.tokenizer.decode(out_tokens[0][0])